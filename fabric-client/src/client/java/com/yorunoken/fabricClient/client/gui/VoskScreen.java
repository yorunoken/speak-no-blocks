package com.yorunoken.fabricClient.client.gui;

import com.yorunoken.fabricClient.client.FabricClientClient;
import com.yorunoken.fabricClient.client.util.SpeechHandler;
import com.yorunoken.fabricClient.client.util.VoskDownloader;
...
public class VoskScreen extends Screen {
    ...
    protected void init() {
        ...
        this.toggleBtn = ButtonWidget.builder(Text.of("Start Voice"), button -> {
                    SpeechHandler handler = FabricClientClient.speechHandler;
                    if (handler.isActive()) {
                        handler.stop();
                    } else {
                        handler.start();
        ...
        this.downloadBtn = ButtonWidget.builder(Text.of("Download Model (1.8G)"), button -> {
                    button.active = false;
                    if (FabricClientClient.speechHandler.isActive()) {
                        FabricClientClient.speechHandler.stop();
                    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String voiceStatus = SpeechHandler.status;
        ...
        if (toggleBtn != null) {
            boolean active = FabricClientClient.speechHandler.isActive();
